...
- Tests never call the real gh. `testrepo.StubGH(t, stdout, stderr, exitCode)` puts a fake `gh` script first on PATH and records its arguments (`Args()`), so tests assert both the rendered output and the exact gh invocation. `testrepo.StubGHCalls(t, first, second, …)` answers successive calls in order (the pull request, then its thread pages) and `Calls()` counts them. To simulate "gh not installed" without losing git, see `gitOnlyPATH` in `cli/github_test.go`.
